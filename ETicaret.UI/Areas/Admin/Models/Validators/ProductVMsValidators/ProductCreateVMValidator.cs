...
namespace ETicaret.UI.Areas.Admin.Models.Validators.ProductVMsValidators
{
    ...
    {
        public ProductCreateVMValidator(IStringLocalizer<ModelResource> localizer)
        {
            ...
            .NotEmpty().WithMessage(localizer["The unit price cannot be ignored!"])
            .NotNull().WithMessage(localizer["The unit price cannot be null!"])
            .GreaterThan(0).WithMessage(localizer["The unit price must be greater than zero!"]);
            ...
            //.NotEmpty().WithMessage(localizer["The category ID cannot be ignored!"])
            //.NotNull().WithMessage(localizer["The category ID cannot be null!"]);
            //RuleFor(x => x.SizeId)
            //.NotEmpty().WithMessage(localizer["The size ID cannot be ignored!"])
            //.NotNull().WithMessage(localizer["The size ID cannot be null!"]);
